...
//  final val notOVERRIDE   = (0L + OVERRIDE) << AntiShift // unused
 ...
 // ------- pickling and unpickling of flags -----------------------------------------------
  ...
  @switch override def flagToString(flag: Long): String = flag match {
  ...
  private[mima] def flagsToString(flags: Long, privateWithin: String): String = {
    ...
    List(nonAccess, access).filter(_ != "").mkString(" ")
  }

  private[mima] def flagsToString(flags: Long): String = {
      ...
      while (i <= MaxBitPosition) {
        ...
        if ((flags & mask) != 0L) {
          ...
          if (s.nonEmpty) {
            ...
            else if (sb.isEmpty) sb.append(s)
